...
- **Incident packets** — after a failed review, CI log, timeout, guard block, or missing artifact, run `"$FO" incident packet <failure-log|-> [--json] [--source-ref ref]`. It deterministically labels the failed trace with local failure cause, MAST category, harness layer, line evidence, source hash, and recovery checks. When the next action needs bounded guidance, run `"$FO" incident recovery <failure-log|incident-json|-> [--json] [--source-ref ref]`; it consumes either the raw log or prior packet JSON, blocks without line evidence, and emits containment, repair, validation, and learning steps. This is the FuguNano cut of HarnessFix + MAST + execution provenance plus PROBE/AIR-style incident response: normalize the failure before repair, memory learning, or Self-Harness mining.
...
> Tooling: `FO=orchestration/fuguectl/fuguectl` — unified driver: `doctor` `init` `version` `preflight` `smoke` `fleet` `task new|log|done|handoff|digest` `guard prompt` `review packet` `incident packet|recovery` `template` `dispatch` `cache` `integrate` `allocate` `skills` `workspace` `agents` `experience` `plan` `goal` `loop` `run` `summary` `self-harness` `runtime` `selftest`.
