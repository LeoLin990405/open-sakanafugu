...
> Tooling: `FO=orchestration/fanout/fuguectl` — unified driver: `doctor` `preflight` `task` `template` `dispatch` `cache` `integrate` `allocate` `skills` `workspace` `plan` `goal` `loop` `run` `summary` `ccb-sync` `selftest`. `orchestration/fanout/fanout` remains a legacy alias.
